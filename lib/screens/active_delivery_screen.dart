  ...
  void _completeDelivery() async {
    if (_otpController.text.length < 4) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter the 4-digit OTP from the customer.')),
      );
      return;
    }
    ...
    try {
      bool success = await _databaseService.completeOrder(widget.orderId, _otpController.text.trim());
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Invalid OTP. Please check with the customer.')),
        );
        setState(() {
          _isCompleting = false;
        });
